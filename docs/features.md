...
- [緯度経度からの周辺駅検索（ekispert_api_get_stations_from_geo）](#緯度経度からの周辺駅検索)
...
**Tool名**: `ekispert_api_get_stations_from_geo`

指定した緯度経度（座標）の周辺にある駅の情報（駅コード、座標、起点からの距離など）を取得します。スマートフォンのGPSや地図アプリから取得した座標を起点に検索できます。

### 呼び出される「駅すぱあと API」

- [緯度経度からの周辺駅検索 - `/geo/station`](https://docs.ekispert.com/v1/api/geo/station.html)

### 利用可能なパラメータ

| パラメータ | 型 | 必須 | 説明 | デフォルト値 |
|----------|-----|-----|------|-----------:|
| `latitude` | string | ○ | 検索の起点となる緯度。-90〜90。十進法度（DD）と度分秒（DMS）のどちらでも指定できます。 | - |
| `longitude` | string | ○ | 検索の起点となる経度。-180〜180。十進法度（DD）と度分秒（DMS）のどちらでも指定できます。 | - |
| `radius` | number | - | 検索半径（単位: m）。指定できる値: 1〜10000の整数。省略時は最寄り駅1件のみを返します。 | - |
| `types` | string[] | - | 検索する駅の交通種別。複数指定可能です。 | 全種別 |
| `stationCount` | number | - | 検索結果の最大回答数。0以上の整数。 | `10` |
| `gcs` | string | - | 座標の測地系。`"tokyo"`（日本測地系）または `"wgs84"`（世界測地系）。 | `"wgs84"` |
| `communityBus` | string | - | 取得結果にコミュニティバスを含めるかどうか。`"contain"`（含める）または `"except"`（含めない）。`types` を省略した場合、または `types` に `bus` もしくは `bus.local` を含む場合のみ有効です。 | `"contain"` |
| `addGateGroup` | string | - | 取得結果に駅の出口情報を含めるかどうか。`"true"`（含める）または `"false"`（含めない）。 | `"false"` |

**入力値の制約**
- `latitude` / `longitude`: **文字列**で指定します。数値ではなく `"35.705333"` のように引用符で囲んでください。
  - 先頭にプラス符号を付けることはできません（`"+35.705333"` はエラーになります）。南緯・西経を表す場合はマイナス符号を使用してください。
- `radius`: 1〜10000 の整数のみ受け付けます。0や小数、10001以上はエラーになります。
- `types`: 指定する場合は1件以上の配列で指定してください。空配列や、指定できない種別を含む場合はエラーになります。
- `stationCount`: 0以上の整数のみ受け付けます。負数や小数はエラーになります。
  - `radius` を省略した場合は最寄り駅1件のみが返却され、`stationCount` の指定は無視されます。
  - `0` を指定した場合は検索結果全件が返却されます。
- `gcs`: `"tokyo"`, `"wgs84"` のいずれかを指定してください。それ以外はエラーになります。入力する座標の測地系と、レスポンスに含まれる座標の測地系の両方に適用されます。
- `communityBus`: `"contain"`, `"except"` のいずれかを指定してください。それ以外はエラーになります。
  - `types` を省略した場合、または `types` に `bus` もしくは `bus.local` を含む場合のみ有効です。それ以外の `types` を指定した場合は無視されます。
- `addGateGroup`: `"true"`, `"false"` のいずれかを指定してください。それ以外はエラーになります。

#### 座標の指定形式

| 形式 | 説明 | 例 |
|---|------|-----|
| 十進法度（DD） | 度を小数で表記します | `"35.705333"` |
| 度分秒（DMS） | 度.分.秒.秒の小数部の形式で表記します | `"35.42.19.20"` |

緯度と経度で異なる形式を指定することもできます。
...
[緯度経度からの周辺駅検索 - `/geo/station` のレスポンス例](https://docs.ekispert.com/v1/api/geo/station.html#example)

#### `addGateGroup: "true"` を指定した場合

`Station` の配下に `GateGroup` が追加され、各出口の名称・座標・起点からの直線距離が返されます。
...
> **補足**: 物件から駅の出口までの徒歩導線を知りたい場合など、出口ごとの距離が必要な場面で `"true"` を指定してください。

### 使用例

- 自然言語での活用シナリオは [使用例](./examples.md#緯度経度からの周辺駅検索) を参照してください。
- 下記はMCPクライアントから直接ツールを呼び出す際の例です。

**座標から最寄り駅を検索**:
```json
{
  "name": "ekispert_api_get_stations_from_geo",
  "arguments": {
    "latitude": "35.705333",
...
```json
{
  "name": "ekispert_api_get_stations_from_geo",
  "arguments": {
    "latitude": "35.42.19.20",
    "longitude": "139.38.59.38"
  }
}
```
...
```
Error: 緯度は文字列で入力してください。数値ではなく "35.705333" のように引用符で囲んで指定します
```

#### 座標の形式が不正な場合

```
Error: 緯度の形式が正しくありません。十進法度（DD、例: 35.705333）または度分秒（DMS、例: 35.42.19.20）の形式で指定してください
```

先頭にプラス符号を付けた場合は、以下のエラーが返されます:

```
Error: 緯度の形式が正しくありません。先頭の + は付けずに指定してください
```
...
エラーではなく、`Point` を含まないレスポンスが返されます:
