...
駅すぱあと APIで利用可能なその他のパラメータについても、今後のアップデートで追加される可能性があります。
...
> **補足**: 駅すぱあと APIがエラーステータスを返した場合、`status` / `message` を含むJSON形式でエラーが返されます。
...
`ekispert_api_search_routes` で `searchType: "plain"`（平均探索）を利用する場合、駅すぱあと APIの仕様に基づき、生成した探索条件の一部のパラメータは以下のように扱われます。
